...
import { QuestionAnswerOptions,
         SubjectTypedTestQuestions,
...
import { ALL_TYPE_QUESTION_FORM_FIELD_OPTIONS,
         ALL_TYPE_QUESTIONS,
         QUESTION_SECTION_VIEW_TYPE,
         QUESTION_SECTION_VIEW_TYPE_FORM_FIELD_OPTIONS } from 'src/constants';

@Component({
  selector: 'app-create-typed-all-type-question',
  templateUrl: './create-typed-all-type-question.component.html',
  styleUrls: ['./create-typed-all-type-question.component.css']
})
export class CreateTypedAllTypeQuestionComponent implements OnInit {
  ...
  ALL_TYPE_QUESTION_FORM_FIELD_OPTIONS = ALL_TYPE_QUESTION_FORM_FIELD_OPTIONS;
  ALL_TYPE_QUESTIONS = ALL_TYPE_QUESTIONS;
  ...
  assertionAnswerChanged(questionId: number, answer: boolean) {
    console.log(answer);
    this.instituteApiService.addUpdateAssertionCorrectAnswer(
      this.currentSubjectSlug,
      questionId.toString(),
      {correct_answer: answer}
    ).subscribe(
      (result: {correct_answer: boolean}) => {
        ...
        this.uiService.showSnackBar('Assertion correct answer updated!', 2000);
        console.log(result);
      },
      errors => {
        if (errors.error) {
