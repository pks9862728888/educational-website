...
import { getOnlyDateFromUnixTimeStamp, getUnixTimeStamp } from 'src/app/format-datepicker';
...
@Component({
  selector: 'app-add-subject-specific-date-test',
  templateUrl: './add-subject-specific-date-test.component.html',
  styleUrls: ['./add-subject-specific-date-test.component.css']
})
export class AddSubjectSpecificDateTestComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.addTestForm = this.formBuilder.group({
      ...
      test_schedule_type: [TEST_SCHEDULE_TYPES.SPECIFIC_DATE, [Validators.required]],
  ...
  initializeForm() {
    ...
    this.addTestForm.patchValue({
      test_schedule_type: TEST_SCHEDULE_TYPES.SPECIFIC_DATE,
  ...
  next(stepper: MatStepper) {
    ...
    const sheduledDateAndTime = getUnixTimeStamp(this.addTestForm.value.date, 23, 59);
  ...
  submit(stepper: MatStepper) {
    ...
    delete data.date;
    data.test_schedule = getUnixTimeStamp(this.addTestForm.value.date, 23, 59);
